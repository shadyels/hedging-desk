...
1. **M1 (scope settled 2026-09-05, ADR-006 Amendment 4):** Barrier option + autocallable under Heston, equity underlyings. **Pricing and MC standard error only — no Greeks**; ADR-008's monitored set is consumed by target publication and lands with M4. Illustrative model parameters (`S0, r, q, v0, κ, θ, ξ, ρ` per underlying) live in an `exo.toml [models.*]` section, **never** in `protocol/refdata/universe.json` — EXO is their only consumer and they are explicitly uncalibrated, so the shared contract surface would lend them an authority they do not have. Product term sheets are `pytest` fixtures; there is no persisted product book until M4's portfolio loop needs one. The discretization scheme decision is resolved in ADR-006 Amendment 4 §4: **QE (Andersen 2008) is the production default scheme**, validated at Feller ratio 0.333 in a convergence study. Full-truncation Euler is retained as an independent cross-check / reference scheme and remains a fully valid, selectable, independently-tested production choice, just not the default.
...
5. **Build the characteristic-function vanilla as a reusable pricer, not a test helper.** There is no
   ...
   twice.
6. **The reproducibility triple is implemented and live.** `exo/src/exo/provenance.py` (305 lines) implements the full capture: `git_sha()` returns the current commit sha (suffixed `-dirty` if the tree has changes), `params_hash(obj)` returns the sha256 hexdigest of a canonical JSON serialization (`sort_keys=True, separators=(",", ":"), allow_nan=False`), and `RunManifest` is a frozen dataclass holding the complete reproducibility record (`schema_version`, `run_id`, `created_ns`, `git_sha`, `model_id`, `params_hash`, `seed`, `n_paths`, `engine`, `params`); it reads from / writes to TOML at `exo/run-manifests/<run_id>.toml`. A real manifest exists at `exo/run-manifests/2026-09-07T21-10-10Z-scheme-convergence.toml`. Root `CLAUDE.md` invariant #7 makes a number without this metadata **invalid**.
