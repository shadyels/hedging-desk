...
## Key invariants (load-bearing for delta-one work)

**Netting & crosses (P1.M3):**
- Per-instrument firm-level external demand: `net_external = Σ_book (target_book − position_book − inflight_book)`.
- The offsetting portion across books is booked as **explicit internal crosses**: two internal trades (one per book side) at the cross reference price, each with `cross_id` (UUIDv7) for compliance lineage. Published on NATS `d1.crosses` and Kafka `posttrade.crosses`.
- Pro-rata allocation of external fills back to books; allocation events on Kafka with full parent-order lineage.
- Core thread validates all inbound targets (via NATS wildcard subscription) against `protocol/refdata/universe.json` before netting; out-of-universe (book, instrument) pairs are **rejected** (root invariant #2 — nowhere to book the position).

**Execution & idempotency (P1.M2–M3):**
- Every `ExecEvent` carries an `ExecId` (`[u8; 20]`), used to dedupe replayed `ExecutionReport`s (root invariant #4). Collisions are impossible: FIX `ExecID`s ≤ 20 bytes are right-aligned; oversize IDs are **hashed** via `DefaultHasher` (never truncated), yielding a 160-bit digest that makes collision astronomically unlikely compared to a dropped fill.
- `ExecOutcome { Duplicate, Applied(Option<Fill>) }` distinguishes replay (already seen) from fill-carrying or terminal-only (reject/cancel/expire) audits, wiring both to the post-trade ledger correctly.

**Ring topology (P1.M2–M5):**
- Core thread drains 8 `rtrb` SPSC rings: FIX pair (outbound `Order`, inbound `ExecEvent`), NATS pair (inbound `Target`, outbound `ExecReport`), feed-ingest ring (`FeedTick`), crosses pair (outbound `CrossRecord`, inbound `TransferRequest`), and Kafka pair (outbound `PostTradeEvent`, consumed by the producer thread).

**Ordered shutdown (P1.M4–M5):**
- Signal core shutdown flag → join core thread (the *only* pusher onto the `PostTradeEvent` ring) → signal producer shutdown → join producer thread.
- This ordering guarantees the producer's final drain is provably complete: all pushes are done, and the drain will see them all.

**Market data priming (P1.M4):**
- Synthetic feed ticks every instrument in the keeper universe, and `run_core` primes `MarketData` synchronously at t=0. No `arrival_mid_px_e9` returns `0` on first tick.

**Tracking analytics (`as_of_ns`) (P1.M5):**
- `as_of_ns` is `window_end_ns` (from the synthetic feed clock), not `Stamper::now_ns`. In deterministic mode `Stamper::now_ns` is a constant, which would collapse the window to zero length.
...
2. **FIX 4.4** (`d1-gateway-fix`): `NewOrderSingle` (35=D), `ExecutionReport` (35=8) inbound, `OrderCancelRequest` (35=F), `OrderCancelReplaceRequest` (35=G). Standard FIX 4.4 semantics only in Phase 1; venue dialects (EMSX/TSOX/FXAll) are thin adapter modules added later — their specs are proprietary and need firm entitlements, so the demo runs against `sim/`'s QuickFIX acceptor. **Note:** 35=F and 35=G message types are listed as scoped message shapes `d1-gateway-fix` is built to handle, but are not yet driven or constructed anywhere in the codebase. See `docs/PONYTAIL-DEBT.md` for the tracked debt item.
