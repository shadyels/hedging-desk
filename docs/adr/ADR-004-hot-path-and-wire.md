...
4. **In-process representation:** plain fixed-size Rust structs; Protobuf types never cross into `d1-core`/`d1-netting` (gateway converts at the edge).
5. **Measurement is a deliverable:** `just bench` produces p50/p99/p99.9 tick-to-order-emit HDR histograms on release builds with pinned cores; these numbers, and only these, go in the showcase deck.
