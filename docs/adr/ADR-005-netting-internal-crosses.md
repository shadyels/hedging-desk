...
1. Compute each book's residual demand: `demand_b = target_b − position_b − inflight_b`.
2. `net_external = Σ_b demand_b` → one external order (or none if inside the instrument's no-trade band).
3. The offsetting portion `Σ_b max(demand_b,0) − max(net_external,0)` (long side; symmetric short side) is matched across books and booked as **internal crosses**: for each matched pair, two internal trade records (buy leg book_x, sell leg book_y), a shared `cross_id`, the cross reference price, and full lineage — published on `posttrade.crosses`.
...
5. External fills are allocated pro-rata to residual demand per book; `posttrade.allocations` records parent `ClOrdID` → book quantities.
