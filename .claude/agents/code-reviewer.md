...
- When a diff DERIVES one quantity from another via an identity (`out = vanilla - in`), the identity test is vacuous by construction — it holds for any `in`, however wrong. Enumerate which BRANCHES of the directly-computed side each test actually reaches: a table-driven closed form had 7 of 8 cells unexercised behind a fully passing suite.
- For a closed form with a validity domain (barrier on the live side, strike ordering, positive time), evaluate OUT-OF-DOMAIN inputs numerically rather than arguing about a missing guard in the abstract. A formula silently returning a sign-violating value (a negative option price) is a far stronger finding than any absent-precondition argument, and lands as a concrete repro.
- When a guard is added to one function and its sibling entry point is left unguarded, check which of the two can actually REACH the bad state. The guard is often on the path that constructs its own consistency (and therefore never needed it) while the unguarded sibling is the one a documented future consumer will call — the public, shared-input entry point is where the check belongs.
- A stability claim of the form "unbiased, so this holds at any N" backed by ONE seed at ONE sample size is untestable from the committed evidence: a single draw consuming half the tolerance budget is equally consistent with a systematic bias that fails at larger N. Run the seed sweep and the sample-size sweep — minutes of compute distinguishes "unlucky draw" from "real bias" definitively. Reason about it and you will be wrong half the time.
- A MEASURED constant guarded by a test-local literal, with the number itself recorded only in a prose comment, re-opens exactly the drift the measurement was meant to close. Check that the value the test asserts and the value the doc records are the SAME OBJECT, not two copies — and that any hand-enumeration sitting beside a measurement has not quietly become the thing readers trust.
- When a fix narrows a function's precondition from "the whole domain" to "a caller-chosen window", ask what the function can no longer verify about that window and who now owns that invariant. The guard can be entirely correct while silently transferring a correctness obligation to every caller — including callers that do not exist yet.
<!-- END learned-lessons -->
