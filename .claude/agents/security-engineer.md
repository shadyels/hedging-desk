...
- When every input validator in a diff uses bare relational comparisons (`<=`/`<`/`>`) with no `math.isfinite`/`np.isnan` companion, treat it as ONE systemic finding and sweep every such site: IEEE-754 makes each relational operator False for NaN, so the guard fails identically everywhere from a single root cause. A type checker gives zero protection here — `float` accepts NaN and Inf statically. Check the guards that are meant to FAIL LOUD hardest: an alignment check of the form `distance > tol` also evaluates False for NaN, so the input takes the "perfectly valid" branch and the safety property inverts precisely where it was strongest.
- For a `Literal`/`Enum`-typed field consumed through a binary `if x == A else <treat as B>` branch, check whether the type checker is the ONLY enforcement. That is adequate while every call site passes a literal constant, and evaporates the moment a config loader or deserializer constructs the same object from dynamic strings — so scope the finding by whether such a loader is on the roadmap, and note that a near-miss value (a case typo) silently selects the other branch rather than erroring.
<!-- END learned-lessons -->
